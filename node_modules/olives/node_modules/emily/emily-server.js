...
requirejs(["Store"], function (Store) {
	...
	exports.config = new Store({
		...
		"CouchDB": {
			hostname: "127.0.0.1",
			// hostname: "10.224.7.243" -- deployment on TP1
			port: 5984
		}
	});
	...
	exports.handlers = new Store({
		...
		"CouchDB" : function (data, onEnd, onData) {
			...
			(data.headers) ? data.headers["Connection"]="close" : data.headers={"Connection": "close"};
			...
			if (data.handshake) {
				...
				cfg.sessionStore.get(cookieJSON["ideafy.sid"].split("s:")[1].split(".")[0], function (err, session) {
					if (err) {
						throw new Error(err);
					} else {
