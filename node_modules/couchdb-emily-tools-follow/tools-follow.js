...
changeHandler = function(payload, onEnd, onData) {
    ...
    if (feeds[payload.path]) feed = feeds[payload.path];
    else {
            url = "http://" + configuration.auth + configuration.hostname + ":" + configuration.port + payload.path;
            payload.query.db = url;
            feed = new follow.Feed(payload.query);
            feeds[payload.path] = feed;
            feed.follow();
    }
