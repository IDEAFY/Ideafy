...
define("Amy/Control-plugin", ["Amy/Event-controller"],
	function(Controller){
		function ControlPluginConstructor(){
			...
			//factorize?
			var _toggle = function(node, className){
			...
			this.radio = function(node, selector, eventName, method, useCapture){
				...
				this.addListener(node, eventName, function(event){
					...
					_current = target;
					that.call(method, event, target);
				}, (useCapture=="true"));
			};

			//delete listener??
			this.toggle = function(node, className, eventName, method, useCapture){
				var that= this;
				this.addListener(node, eventName, function(event){
					var target = event.target;
					that.call(method, event, _toggle(target, className));
				}, (useCapture=="true"));
			};
		}
